...
void OLED_ShowCHinese(uint8_t x,uint8_t y,uint8_t no, uint8_t Color_Turn)
{
    ...
    for(t=0;t<16;t++)
		{
				if (Color_Turn)
					OLED_WR_DATA(~tfont16[2*no][t]); //显示汉字的上半部分
				else
					OLED_WR_DATA(tfont16[2*no][t]); //显示汉字的上半部分
        }
    ...
    for(t=0;t<16;t++)
		{
				if (Color_Turn)
					OLED_WR_DATA(~tfont16[2*no+1][t]); //显示汉字的上半部分
				else
					OLED_WR_DATA(tfont16[2*no+1][t]);//显示汉字的上半部分
...
//	  OLED_ShowString(4,32,"光照",16,0);
//         OLED_ShowNum(4,36, adc_volt,4,16,0);
}

void OLED_Auto_Screen(void) {
    OLED_Clear();
	  OLED_ShowString(16,0,"自动模式中",16,0);
//	  OLED_ShowString(96,0, "!!",16);
	  OLED_ShowString(16,16,"温度",16,0);
         OLED_ShowNum(64,16,temperature,2,16,0);
    OLED_ShowString(16,32,"湿度",16,0);
         OLED_ShowNum(64,20, humidity,2,16,0);
	  OLED_ShowString(16,48,"光照",16,0);
         OLED_ShowNum(64,36, adc_volt,2,16,0);
}
//void Show_Mode_Select_Screen(void) {
//    OLED_Clear();
//    OLED_ShowString(32, 8,  "选  择  模  式",16,0);
//    OLED_ShowString(32, 8, "手  动  模  式", 16,0);
//    OLED_ShowString(32, 32, "自  动  模  式",16,0);
//}
...
void OLED_Manual_Mode_Screen(void) {
    ...
    switch (system_state.manual_page) {
            ...
            OLED_ShowString(40,8,"灯",16,0);
            OLED_ShowString(80,8, system_state.led_state ? "开 " : "关", 16,0);
            OLED_ShowString(40,26, "风扇",16, 0);
            OLED_ShowString(40, 44, "水泵",16, 0);
            break;
        case MANUAL_PAGE_MOTOR_A:
		    OLED_ShowString(40,8,"灯",16,0);
            OLED_ShowString(40,26, "风扇",16, 0);
			OLED_ShowString(80,26, system_state.motor_a_state ? "开 " : "关",16, 0);
            OLED_ShowString(40, 44, "水泵",16, 0);
            break;
        case MANUAL_PAGE_MOTOR_B:  
            ...
            OLED_ShowString(40,26, "风扇", 16,0);
            OLED_ShowString(40, 44, "水泵", 16,0);
			OLED_ShowString(80,44, system_state.motor_b_state ? "开 " : "关", 16,0);
            break;
    }
}
void I2C_Scan(void)
